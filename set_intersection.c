...
const int cyclic_shift1 = _MM_SHUFFLE(0, 3, 2, 1); // rotating right
const int cyclic_shift2 = _MM_SHUFFLE(2, 1, 0, 3); // rotating left
const int cyclic_shift3 = _MM_SHUFFLE(1, 0, 3, 2); // between
static __m128i all_zero_si128;
static __m128i all_one_si128;
void setup_simd_constants()
{
    all_zero_si128 = _mm_setzero_si128();
    all_one_si128 = _mm_set_epi32(0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff);
}
                 ...
                 int v_base, PackState v_bit)
{
    ...
    int i = 0;
    while (i < size_a && bases_a[i] < v_base)
        ++i;
    if (i == size_a)
    {
        bases_a[i] = v_base;
